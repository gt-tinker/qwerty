...
struct FuncSpec {
  ...
  bool operator==(const FuncSpec &other) const {
    return other.is_adj == is_adj && other.n_controls == n_controls;
  }

  bool operator<(const FuncSpec &other) const {
    return static_cast<int>(other.is_adj) < static_cast<int>(is_adj) ||
           (other.is_adj == is_adj && other.n_controls < n_controls);
  }

  bool isCallableFromPython() const { return !is_adj && !n_controls; }
  ...
  std::string getSymName(llvm::StringRef og_func_name) const {
    if (isCallableFromPython()) {
      ...
      return (og_func_name + "__" + (is_adj ? "adj" : "fwd") +
  ...
  qcirc::FuncSpecAttr getAttr(mlir::MLIRContext *ctx,
    ...
    return qcirc::FuncSpecAttr::get(
        ...
        mlir::FlatSymbolRefAttr::get(ctx, getSymName(og_func_name)), func_type);
  }
};
...
// the CGO paper)
mlir::LogicalResult findFuncUsages(mlir::ModuleOp module_op,
                                   FuncUsages &usages) {
  // Functions that have been seen in func_const ops, indexed by type
  llvm::DenseMap<mlir::FunctionType, llvm::SmallVector<llvm::StringRef>>
      func_types;
  ...
  for (qwerty::FuncOp func :
    ...
    if (func.isPublic()) {
      ...
      assert(!usages.count(name) && "Duplicate symbol names?");
      usages[name].specs_needed.insert(
          FuncSpec(/*is_adj=*/false, /*n_controls=*/0));
    }

    auto func_sym_uses_opt = mlir::SymbolTable::getSymbolUses(func, module_op);
  ...
  module_op->walk([&](qwerty::CallIndirectOp calli) {
    ...
    if (!lattice_val.is_bottom) {
      ...
      mlir::FunctionType callee_func_type = mlir::FunctionType::get(
          module_op.getContext(), calli.getCallOperands().getTypes(),
          calli.getResults().getTypes());
      if (func_types.count(callee_func_type)) {
        for (llvm::StringRef func_name : func_types.at(callee_func_type)) {
      ...
      if (calli.getCallOperands().size() == 1 &&
        ...
        for (auto [name, n_callee_qubits] : rev_funcs) {
          ...
          if (n_call_qubits == n_callee_qubits) {
            if (adjointed_funcs_bottom ||
                adjointed_funcs.count(maybe_callee_name)) {
            ...
            if (predded_funcs_bottom ||
                predded_funcs.count(maybe_callee_name)) {
              usages[maybe_callee_name].specs_needed.insert(
  ...
  DEBUG_WITH_TYPE("find-func-usages", {
    ...
    for (auto &[sym, sym_usage] : usages) {
      ...
      llvm::interleaveComma(
          sym_usage.specs_needed, llvm::errs(), [](auto &spec) {
...
mlir::FunctionType convertFuncType(qwerty::FuncOp func,
  ...
  mlir::Type func_type_with_captures = mlir::FunctionType::get(
      ctx, func.getBody().getArgumentTypes(),
      func.getQwertyFuncType().getFunctionType().getResults());
  mlir::FunctionType new_func_type = llvm::dyn_cast<mlir::FunctionType>(
...
mlir::LogicalResult generateFuncSpecs(mlir::ModuleOp module_op,
                                      FuncUsages &usages,
                                      const mlir::TypeConverter &type_conv) {
  ...
  for (qwerty::FuncOp func : func_queue) {
    ...
    for (auto [i, spec] : llvm::enumerate(usage.specs_needed)) {
      ...
      if (!spec.n_controls) {
        ...
        llvm::SmallVector<mlir::Type> new_inputs(inner_func_ty.getInputs());
        llvm::SmallVector<mlir::Type> new_results(inner_func_ty.getResults());
        assert(
            func.getQwertyFuncType().getReversible() && !new_inputs.empty() &&
            !new_results.empty() &&
            new_inputs[new_inputs.size() - 1] ==
                new_results[new_results.size() - 1] &&
            llvm::isa<qwerty::QBundleType>(new_inputs[new_inputs.size() - 1]) &&
            "Invariant of reversible functions violated");
        qwerty::QBundleType qbundle_ty =
            llvm::cast<qwerty::QBundleType>(new_inputs[new_inputs.size() - 1]);
        bigger_qbundle_ty = rewriter.getType<qwerty::QBundleType>(
        ...
        new_func_type = rewriter.getType<qwerty::FunctionType>(
            rewriter.getType<mlir::FunctionType>(new_inputs, new_results),
            /*reversible=*/true);
      }

      std::string new_func_name = spec.getSymName(func.getSymName());
      rewriter.setInsertionPoint(func);
      qwerty::FuncOp new_func =
          rewriter.create<qwerty::FuncOp>(loc, new_func_name, new_func_type);
      // Teensy tiny optimization: steal the body of the function for the
      // last specialization instead of needlessly cloning it
      if (last) {
        rewriter.inlineRegionBefore(func.getBody(), new_func.getBody(),
                                    new_func.getBody().end());
      } else {
        rewriter.cloneRegionBefore(func.getBody(), new_func.getBody(),
                                   new_func.getBody().end());
      }
      ...
      if (spec.n_controls) {
        ...
        qwerty::BasisAttr pred_basis =
            qwerty::BasisAttr::getAllOnesBasis(func.getContext(), pred_dim);
        if (mlir::failed(qwerty::predicateBlockInPlaceFixTerm<qwerty::ReturnOp>(
      ...
      if (spec.is_adj) {
        if (mlir::failed(qcirc::takeAdjointOfBlockInPlace<qwerty::ReturnOp>(
                rewriter, *block, loc))) {
          return func->emitOpError("Taking adjoint failed");
        }
      }

      mlir::FunctionType qcirc_func_type = convertFuncType(new_func, type_conv);
      spec_attrs.push_back(
          spec.getAttr(func->getContext(), func.getSymName(), qcirc_func_type));
    }
    ...
    if (usage.taken_as_const) {
      ...
      if (mlir::failed(
              type_conv.convertTypes(capture_types, qcirc_capture_types))) {
        return func->emitOpError("Cannot convert capture types");
      }

      qcirc::CallableMetadataOp meta =
          rewriter.create<qcirc::CallableMetadataOp>(
              loc, FuncSpec::getMetadataSymName(func.getSymName()),
...
public:
  QwertyToQCircTypeConverter(mlir::MLIRContext *ctx) {
    ...
    addConversion([=](qwerty::QBundleType qbundle) {
      return qcirc::ArrayType::get(ctx, qcirc::QubitType::get(ctx),
                                   qbundle.getDim());
    });
    addConversion([=](qwerty::BitBundleType bit_bundle) {
      return qcirc::ArrayType::get(ctx, mlir::IntegerType::get(ctx, 1),
                                   bit_bundle.getDim());
    });
    ...
    addConversion(
        [=](qwerty::FunctionType func_ty) -> std::optional<mlir::Type> {
...
struct SCFIfOpTypeFix : public mlir::OpConversionPattern<mlir::scf::IfOp> {
  ...
  mlir::LogicalResult
  matchAndRewrite(mlir::scf::IfOp if_op, OpAdaptor adaptor,
    ...
    if (mlir::failed(type_conv->convertTypes(if_op->getResultTypes(),
                                             new_result_types))) {
    ...
    mlir::scf::IfOp new_if_op = rewriter.create<mlir::scf::IfOp>(
        loc, new_result_types, adaptor.getCondition());
    rewriter.inlineRegionBefore(if_op.getThenRegion(),
                                new_if_op.getThenRegion(),
                                new_if_op.getThenRegion().end());
    if (mlir::failed(rewriter.convertRegionTypes(&new_if_op.getThenRegion(),
                                                 *type_conv))) {
      return rewriter.notifyMatchFailure(if_op,
    ...
    if (mlir::failed(rewriter.convertRegionTypes(&new_if_op.getElseRegion(),
                                                 *type_conv))) {
      return rewriter.notifyMatchFailure(if_op,
...
struct SCFYieldOpTypeFix
    : public mlir::OpConversionPattern<mlir::scf::YieldOp> {
  using mlir::OpConversionPattern<mlir::scf::YieldOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(mlir::scf::YieldOp yield, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    rewriter.replaceOpWithNewOp<mlir::scf::YieldOp>(yield,
                                                    adaptor.getResults());
    return mlir::success();
  }
};
...
struct ArithSelectOpTypeFix
  ...
  mlir::LogicalResult
  matchAndRewrite(mlir::arith::SelectOp select, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    rewriter.replaceOpWithNewOp<mlir::arith::SelectOp>(
        select, adaptor.getCondition(), adaptor.getTrueValue(),
...
struct CalcOpTypeFix : public mlir::OpConversionPattern<qcirc::CalcOp> {
  ...
  mlir::LogicalResult
  matchAndRewrite(qcirc::CalcOp calc, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    const mlir::TypeConverter *type_conv = getTypeConverter();
    if (!type_conv) {
    ...
    llvm::SmallVector<mlir::Type> new_result_types;
    if (mlir::failed(type_conv->convertTypes(calc->getResultTypes(),
                                             new_result_types))) {
    ...
    qcirc::CalcOp new_calc = rewriter.create<qcirc::CalcOp>(
        loc, new_result_types, adaptor.getInputs());
    rewriter.inlineRegionBefore(calc.getRegion(), new_calc.getRegion(),
                                new_calc.getRegion().end());
    if (mlir::failed(
            rewriter.convertRegionTypes(&new_calc.getRegion(), *type_conv))) {
      return rewriter.notifyMatchFailure(calc,
                                         "Converting then region types failed");
    }
...
struct CalcYieldOpTypeFix
    : public mlir::OpConversionPattern<qcirc::CalcYieldOp> {
  using mlir::OpConversionPattern<qcirc::CalcYieldOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qcirc::CalcYieldOp yield, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    rewriter.replaceOpWithNewOp<qcirc::CalcYieldOp>(yield,
                                                    adaptor.getResults());
    return mlir::success();
  }
};

struct QBundlePackOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundlePackOp> {
  using mlir::OpConversionPattern<qwerty::QBundlePackOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundlePackOp pack, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
...
struct QBundleUnpackOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleUnpackOp> {
  using mlir::OpConversionPattern<qwerty::QBundleUnpackOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleUnpackOp unpack, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    rewriter.replaceOpWithNewOp<qcirc::ArrayUnpackOp>(unpack,
                                                      adaptor.getQbundle());
    return mlir::success();
  }
};

struct BitBundlePackOpLowering
    : public mlir::OpConversionPattern<qwerty::BitBundlePackOp> {
  using mlir::OpConversionPattern<qwerty::BitBundlePackOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::BitBundlePackOp pack, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
...
struct BitBundleUnpackOpLowering
    : public mlir::OpConversionPattern<qwerty::BitBundleUnpackOp> {
  using mlir::OpConversionPattern<
      qwerty::BitBundleUnpackOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::BitBundleUnpackOp unpack, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    rewriter.replaceOpWithNewOp<qcirc::ArrayUnpackOp>(unpack,
                                                      adaptor.getBundle());
    return mlir::success();
...
struct FuncOpLowering : public mlir::OpConversionPattern<qwerty::FuncOp> {
  ...
  mlir::LogicalResult
  matchAndRewrite(qwerty::FuncOp func, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    const mlir::TypeConverter *type_conv = getTypeConverter();
    if (!type_conv) {
    ...
    mlir::func::FuncOp new_func = rewriter.create<mlir::func::FuncOp>(
        loc, func.getSymName(), new_func_type);
    if (func.isPrivate()) {
      new_func.setPrivate();
    }
    ...
    if (mlir::failed(
            rewriter.convertRegionTypes(&new_func.getBody(), *type_conv))) {
...
struct ReturnOpLowering : public mlir::OpConversionPattern<qwerty::ReturnOp> {
  ...
  mlir::LogicalResult
  matchAndRewrite(qwerty::ReturnOp ret, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
...
struct CallOpLowering : public mlir::OpConversionPattern<qwerty::CallOp> {
  ...
  mlir::LogicalResult
  matchAndRewrite(qwerty::CallOp call, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    const mlir::TypeConverter *type_conv = getTypeConverter();
    if (!type_conv) {
    ...
    llvm::SmallVector<mlir::Type> new_result_types;
    if (mlir::failed(type_conv->convertTypes(call.getResults().getTypes(),
                                             new_result_types))) {
    ...
    if (call.getPred() && !call.getPredAttr().hasOnlyOnes()) {
      return rewriter.notifyMatchFailure(
          call, "Predicate is not all 1s. Is -only-pred-ones broken (or did "
                "it not run?)");
    }

    std::string new_sym_name = FuncSpec(call).getSymName(call.getCallee());
    rewriter.replaceOpWithNewOp<mlir::func::CallOp>(
        call, new_sym_name, new_result_types, adaptor.getCapturesAndOperands());
    return mlir::success();
  }
};

struct CallIndirectOpLowering
    : public mlir::OpConversionPattern<qwerty::CallIndirectOp> {
  using mlir::OpConversionPattern<qwerty::CallIndirectOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::CallIndirectOp call, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    rewriter.replaceOpWithNewOp<qcirc::CallableInvokeOp>(
        call, adaptor.getCallee(), adaptor.getCallOperands());
    return mlir::success();
  }
};

struct FuncConstOpLowering
    : public mlir::OpConversionPattern<qwerty::FuncConstOp> {
  using mlir::OpConversionPattern<qwerty::FuncConstOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::FuncConstOp func_const, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    const mlir::TypeConverter *type_conv = getTypeConverter();
    if (!type_conv) {
      return rewriter.notifyMatchFailure(func_const, "Need a type converter");
    }
    qcirc::CallableType func_ty;
    if (!(func_ty = type_conv->convertType<qcirc::CallableType>(
              func_const.getResult().getType()))) {
      return rewriter.notifyMatchFailure(
          func_const, "Could not convert result type of qwerty.func_const to a "
                      "CallableType");
    }
    std::string metadata_sym =
        FuncSpec::getMetadataSymName(func_const.getFunc());

    rewriter.replaceOpWithNewOp<qcirc::CallableCreateOp>(
        func_const, func_ty, metadata_sym, adaptor.getCaptures());
    return mlir::success();
  }
};

struct FuncAdjointOpLowering
    : public mlir::OpConversionPattern<qwerty::FuncAdjointOp> {
  using mlir::OpConversionPattern<qwerty::FuncAdjointOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::FuncAdjointOp func_adj, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
...
struct FuncPredOpLowering
    : public mlir::OpConversionPattern<qwerty::FuncPredOp> {
  using mlir::OpConversionPattern<qwerty::FuncPredOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::FuncPredOp func_pred, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    ...
    rewriter.replaceOpWithNewOp<qcirc::CallableControlOp>(
        func_pred, adaptor.getCallee(), func_pred.getPred().getDim());
    return mlir::success();
  }
};
...
struct BitInitOpLowering : public mlir::OpConversionPattern<qwerty::BitInitOp> {
  ...
  mlir::LogicalResult
  matchAndRewrite(qwerty::BitInitOp prep, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    mlir::Location loc = prep.getLoc();

    mlir::ValueRange bits =
        rewriter.create<qwerty::BitBundleUnpackOp>(loc, prep.getBitBundle())
            .getBits();
    mlir::ValueRange input_unpacked =
        rewriter.create<qwerty::QBundleUnpackOp>(loc, prep.getQbundleIn())
            .getQubits();
    ...
    for (size_t i = 0; i < bits.size(); i++) {
      ...
      mlir::scf::IfOp if_op = rewriter.create<mlir::scf::IfOp>(
          loc, rewriter.getType<qcirc::QubitType>(), bit, true);
...

      {
        ...
        mlir::Value flipped =
            rewriter
                .create<qcirc::Gate1QOp>(loc, qcirc::Gate1Q::X,
...
struct QBundleInitOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleInitOp> {
  using mlir::OpConversionPattern<qwerty::QBundleInitOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleInitOp init, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    ...
    mlir::ValueRange unpacked =
        rewriter.create<qwerty::QBundleUnpackOp>(loc, init.getQbundleIn())
            .getQubits();
    llvm::SmallVector<mlir::Value> qubits(unpacked.begin(), unpacked.end());
    if (basis.getDim() != qubits.size()) {
      return rewriter.notifyMatchFailure(
          init, "basis size does not match input bundle size");
    }
    ...
    for (uint64_t i = 0; i < basis.getElems().size(); i++) {
      ...
      if (elem.getStd()) {
        return rewriter.notifyMatchFailure(
            init, "Passing a standard basis to qbinit makes no sense");
      } else if (elem.getVeclist()) {
        if (elem.getVeclist().getVectors().size() != 1) {
          return rewriter.notifyMatchFailure(
              init, "Expected a singleton basis for qbinit");
        }
        qwerty::BasisVectorAttr vec = elem.getVeclist().getVectors()[0];
        if (vec.hasPhase()) {
          qubits[qubit_idx] = rewriter
                                  .create<qcirc::Gate1QOp>(
                                      loc, qcirc::Gate1Q::X, mlir::ValueRange(),
                                      qubits[qubit_idx])
                                  .getResult();
          qubits[qubit_idx] =
              rewriter
                  .create<qcirc::Gate1Q1POp>(
                      loc, qcirc::Gate1Q1P::P, basis_phases[phase_idx++],
                      mlir::ValueRange(), qubits[qubit_idx])
                  .getResult();
          qubits[qubit_idx] = rewriter
                                  .create<qcirc::Gate1QOp>(
                                      loc, qcirc::Gate1Q::X, mlir::ValueRange(),
...
struct QBundleDeinitOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleDeinitOp> {
  using mlir::OpConversionPattern<qwerty::QBundleDeinitOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleDeinitOp deinit, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    ...
    mlir::ValueRange unpacked =
        rewriter.create<qwerty::QBundleUnpackOp>(loc, deinit.getQbundleIn())
            .getQubits();
    llvm::SmallVector<mlir::Value> qubits(unpacked.begin(), unpacked.end());
    if (basis.getDim() != qubits.size()) {
      return rewriter.notifyMatchFailure(
          deinit, "basis size does not match input bundle size");
    }
    ...
    for (uint64_t i = 0; i < basis.getElems().size(); i++) {
      ...
      if (elem.getStd()) {
        return rewriter.notifyMatchFailure(
            deinit, "Passing a standard basis to qbinit makes no sense");
      } else if (elem.getVeclist()) {
        if (elem.getVeclist().getVectors().size() != 1) {
          return rewriter.notifyMatchFailure(
              deinit, "Expected a singleton basis for qbinit");
        }
        qwerty::BasisVectorAttr vec = elem.getVeclist().getVectors()[0];
        for (uint64_t j = 0; j < vec.getDim(); j++) {
          uint64_t bit = vec.getEigenbits()[vec.getDim() - j - 1];
        ...
        if (vec.hasPhase()) {
          ...
          mlir::Value neg_theta = wrapStationaryFloatOps(
              ...
              [&](mlir::ValueRange args) {
                ...
                return rewriter.create<mlir::arith::NegFOp>(loc, theta_arg)
                    .getResult();
              });
          qubits[qubit_idx] = rewriter
                                  .create<qcirc::Gate1Q1POp>(
                                      loc, qcirc::Gate1Q1P::P, neg_theta,
                                      mlir::ValueRange(), qubits[qubit_idx])
                                  .getResult();
          qubits[qubit_idx] = rewriter
                                  .create<qcirc::Gate1QOp>(
...
struct TrivialQBundlePrepOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundlePrepOp> {
  using mlir::OpConversionPattern<qwerty::QBundlePrepOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundlePrepOp prep, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    mlir::Location loc = prep.getLoc();
...
struct NontrivialQBundlePrepOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundlePrepOp> {
  using mlir::OpConversionPattern<qwerty::QBundlePrepOp>::OpConversionPattern;
  ...
  matchAndRewrite(qwerty::QBundlePrepOp prep, OpAdaptor adaptor,
    ...
    if (prep.getPrimBasis() == qwerty::PrimitiveBasis::Z &&
      ...
      mlir::Value zeros = rewriter
                              .create<qwerty::QBundlePrepOp>(
                                  loc, qwerty::PrimitiveBasis::Z,
                                  qwerty::Eigenstate::PLUS, prep.getDim())
                              .getResult();
          ...
          rewriter.getAttr<qwerty::BasisElemAttr>(
              rewriter.getAttr<qwerty::BasisVectorListAttr>(
                  rewriter.getAttr<qwerty::BasisVectorAttr>(
                      prep.getPrimBasis(), prep.getEigenstate(), prep.getDim(),
                      false))));
      rewriter.replaceOpWithNewOp<qwerty::QBundleInitOp>(
          prep, basis, mlir::ValueRange(), zeros);
      return mlir::success();
    }
  }
};

struct QBundleDiscardOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleDiscardOp> {
  using mlir::OpConversionPattern<
      qwerty::QBundleDiscardOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleDiscardOp discard, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    mlir::Location loc = discard.getLoc();
    uint64_t dim = discard.getQbundle().getType().getDim();
    // Hopefully canonicalization will get rid of this guy
    mlir::ValueRange unpacked =
        rewriter.create<qwerty::QBundleUnpackOp>(loc, discard.getQbundle())
            .getQubits();
...
struct QBundleDiscardZeroOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleDiscardZeroOp> {
  using mlir::OpConversionPattern<
      qwerty::QBundleDiscardZeroOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleDiscardZeroOp discard, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    mlir::Location loc = discard.getLoc();
    uint64_t dim = discard.getQbundle().getType().getDim();
    // Hopefully canonicalization will get rid of this guy
...
struct QBundlePhaseOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundlePhaseOp> {
  using mlir::OpConversionPattern<qwerty::QBundlePhaseOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundlePhaseOp phaseOp, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    mlir::Location loc = phaseOp.getLoc();
    mlir::ValueRange unpacked =
        rewriter.create<qwerty::QBundleUnpackOp>(loc, phaseOp.getQbundleIn())
            .getQubits();
    mlir::Value victim = unpacked[0];
    victim = rewriter
                 .create<qcirc::Gate1Q1POp>(loc, qcirc::Gate1Q1P::P,
                                            phaseOp.getTheta(),
                                            mlir::ValueRange(), victim)
                 .getResult();
    victim = rewriter
                 .create<qcirc::Gate1QOp>(loc, qcirc::Gate1Q::X,
                                          mlir::ValueRange(), victim)
...
struct QBundleIdentityOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleIdentityOp> {
  using mlir::OpConversionPattern<
      qwerty::QBundleIdentityOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleIdentityOp id, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
...
void computeAngles(std::complex<double> alpha, std::complex<double> beta,
...
void revisedUndoSubroutine(
    ...
    Eigen::SparseVector<std::complex<double>> &state_vector, uint64_t dim) {
  ...
  if (dim == 1) {
    ...
    double phi = 0.0;
    double gamma = 0.0;
    computeAngles(state_vector.coeff(0), state_vector.coeff(1), gamma, theta,
                  phi);
    ...
    Eigen::SparseVector<std::complex<double>> state_vector_prime(
        std::pow(2, dim - 1));
    Eigen::SparseVector<double> theta_vec_n(std::pow(2, dim - 1));
    Eigen::SparseVector<double> phi_vec_n(std::pow(2, dim - 1));
    size_t skip_idx =
        state_vector.size(); // Initialized with Size if no skipping was needed,
                             // no false skipping happens
    for (Eigen::SparseVector<std::complex<double>>::InnerIterator it(
             state_vector);
         it; ++it) {
      size_t idx = it.index();
      if (idx == skip_idx) {
      ...
      if (idx &
          1) { // odd case. alpha is alpha2jplus1 right now and alpha2j == 0
        alpha_2j = 0;
        alpha_2jplus1 = alpha;
      } else { // even case
        alpha_2j = alpha;
      ...
      std::complex<double> alpha_2jplus1_sqr =
          std::pow(std::abs(alpha_2jplus1), 2);
      ...
      double theta_j = 0.0;
      double phi_j = 0.0;
      computeAngles(alpha_2j_prime, alpha_2jplus1_prime, gamma_j, theta_j,
                    phi_j);
      state_vector_prime.coeffRef(j) = r_j * std::polar(1.0, gamma_j);

      // Add gates if necessary to undo
      if (std::abs(theta_j - 0) <= ATOL)
        continue;
...
void recrunchAngles(Eigen::SparseVector<double> &angles_in,
...
void multiplexedRyRzSynthesis(mlir::RewriterBase &rewriter, mlir::Location &loc,
                              qcirc::Gate1Q1P rot_kind, uint64_t dim,
                              llvm::SmallVector<mlir::Value> &qubits,
  ...
  if (dim == 1) {
    ...
    qubits[0] = rewriter
                    .create<qcirc::Gate1Q1POp>(loc, rot_kind, theta_const,
                                               no_controls, qubits[0])
                    .getResult();
  } else { // dim > 1
    {
      Eigen::SparseVector<double> thetas_prime(thetas.size() / 2);
      recrunchAngles(thetas, thetas_prime,
                     [](double theta_2k, double theta_2kplus1) {
                       return (theta_2k + theta_2kplus1) / 2;
                     });
      // Exclude second-to-last qubit
      llvm::SmallVector<mlir::Value> qubits_prime(
          qubits.begin(), qubits.begin() + qubits.size() - 2);
      qubits_prime.push_back(qubits[qubits.size() - 1]);
      multiplexedRyRzSynthesis(rewriter, loc, rot_kind, dim - 1, qubits_prime,
                               thetas_prime);
    ...
    qcirc::Gate1QOp X = rewriter.create<qcirc::Gate1QOp>(
        loc, qcirc::Gate1Q::X, qubits[dim - 2], qubits[dim - 1]);
...

    {
      ...
      // Exclude second-to-last qubit
      llvm::SmallVector<mlir::Value> qubits_double_prime(
          qubits.begin(), qubits.begin() + qubits.size() - 2);
      qubits_double_prime.push_back(qubits[qubits.size() - 1]);
      multiplexedRyRzSynthesis(rewriter, loc, rot_kind, dim - 1,
                               qubits_double_prime, thetas_double_prime);
      // Thread qubits through
      for (size_t i = 0; i < qubits_double_prime.size() - 1; i++) {
        qubits[i] = qubits_double_prime[i];
      }
      qubits[qubits.size() - 1] =
          qubits_double_prime[qubits_double_prime.size() - 1];
    }

    X = rewriter.create<qcirc::Gate1QOp>(loc, qcirc::Gate1Q::X, qubits[dim - 2],
                                         qubits[dim - 1]);
    qubits[dim - 2] = X.getControlResults()[0];
    qubits[dim - 1] = X.getResult();
  }
...
void arbitraryStatePrep(mlir::RewriterBase &rewriter, mlir::Location loc,
  ...
  for (qwerty::SuperposElemAttr elem : superpos.getElems()) {
    ...
    state_vector.coeffRef(eigenbits.getZExtValue()) =
        std::sqrt(prob) * std::polar(1.0, phase);
  }
  ...
  for (size_t i = 1; i <= dim; i++) {
    llvm::SmallVector<mlir::Value> target_qubits(qubits.begin(),
                                                 qubits.begin() + i);

    multiplexedRyRzSynthesis(rewriter, loc, qcirc::Gate1Q1P::Ry, i,
                             target_qubits, theta_vecs[i - 1]);
    multiplexedRyRzSynthesis(rewriter, loc, qcirc::Gate1Q1P::Rz, i,
                             target_qubits, phi_vecs[i - 1]);
...
void uniformStatePrep(mlir::RewriterBase &rewriter, mlir::Location loc,
                      llvm::SmallVectorImpl<mlir::Value> &qubits) {
  for (size_t i = 0; i < qubits.size(); i++) {
    qubits[i] = rewriter
                    .create<qcirc::Gate1QOp>(loc, qcirc::Gate1Q::H,
                                             mlir::ValueRange(), qubits[i])
                    .getResult();
  }
}
...
void quasiGhzStatePrep(mlir::RewriterBase &rewriter, mlir::Location loc,
                       qwerty::SuperposAttr superpos,
                       llvm::SmallVectorImpl<mlir::Value> &qubits) {
  ...
  mlir::Value theta_const = qcirc::stationaryF64Const(rewriter, loc, theta);
  qubits[0] =
      rewriter
          .create<qcirc::Gate1Q1POp>(loc, qcirc::Gate1Q1P::Ry, theta_const,
                                     mlir::ValueRange(), qubits[0])
          .getResult();
  ...
  if (std::abs(relative_phase - 0.0) >= ATOL) {
    ...
    qubits[0] =
        rewriter
            .create<qcirc::Gate1Q1POp>(loc, qcirc::Gate1Q1P::P, phi_const,
                                       mlir::ValueRange(), qubits[0])
            .getResult();
  }
}

struct SuperposOpLowering
    : public mlir::OpConversionPattern<qwerty::SuperposOp> {
  using mlir::OpConversionPattern<qwerty::SuperposOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::SuperposOp superPosOp, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    ...
    for (qwerty::BasisVectorAttr elem_vec : vecs) {
      desired_prim_bases.push_back(rewriter.getAttr<qwerty::BasisElemAttr>(
          rewriter.getAttr<qwerty::BuiltinBasisAttr>(elem_vec.getPrimBasis(),
                                                     elem_vec.getDim())));
    }
    qwerty::BasisAttr desired_basis =
        rewriter.getAttr<qwerty::BasisAttr>(desired_prim_bases);
...
struct EnsembleOpLowering
    : public mlir::OpConversionPattern<qwerty::EnsembleOp> {
  using mlir::OpConversionPattern<qwerty::EnsembleOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::EnsembleOp ensemble, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    ...
    for (auto [i, superpos_elem] : llvm::enumerate(superpos_attr.getElems())) {
      ...
      qwerty::SuperposElemAttr new_superpos_elem =
          rewriter.getAttr<qwerty::SuperposElemAttr>(
              superpos_elem.getProb(), superpos_elem.getPhase(), new_vectors);
    ...
    mlir::Value superpos =
        rewriter.create<qwerty::SuperposOp>(loc, new_superpos_attr)
            .getQbundle();

    mlir::ValueRange unpacked =
        rewriter.create<qwerty::QBundleUnpackOp>(loc, superpos).getQubits();
    assert(unpacked.size() > n_extra_bits && "Superpos bundle is too small");

    llvm::SmallVector<mlir::Value> extra_qubits(
        unpacked.begin(), unpacked.begin() + n_extra_bits);
    llvm::SmallVector<mlir::Value> ensemble_qubits(
        unpacked.begin() + n_extra_bits, unpacked.end());

    mlir::Value extra_qbundle =
        rewriter.create<qwerty::QBundlePackOp>(loc, extra_qubits).getQbundle();
    mlir::Value ensemble_qbundle =
        rewriter.create<qwerty::QBundlePackOp>(loc, ensemble_qubits)
            .getQbundle();
...
qwerty::BasisElemAttr rebuildZ(mlir::RewriterBase &rewriter,
  ...
  if (bib.getPrimBasis() == qwerty::PrimitiveBasis::Z) {
    ...
    return rewriter.getAttr<qwerty::BasisElemAttr>(
        rewriter.getAttr<qwerty::BuiltinBasisAttr>(qwerty::PrimitiveBasis::Z,
                                                   bib.getDim()));
  }
}
...
qwerty::BasisElemAttr rebuildZ(mlir::RewriterBase &rewriter,
  ...
  if (veclist.getPrimBasis() == qwerty::PrimitiveBasis::Z &&
      !veclist.hasPhases()) {
...
qwerty::BuiltinBasisAttr splitStd(mlir::RewriterBase &rewriter,
  ...
  qwerty::BasisElemAttr remainder = rewriter.getAttr<qwerty::BasisElemAttr>(
      rewriter.getAttr<qwerty::BuiltinBasisAttr>(bib.getPrimBasis(),
                                                 bib.getDim() - new_dim));
...
qwerty::BasisElemAttr factorFull(mlir::RewriterBase &rewriter,
  ...
  for (qwerty::BasisVectorAttr vec : vl.getVectors()) {
    ...
    size_t prefix =
        eigenbits.extractBitsAsZExtValue(std_dim, n_qubits - std_dim);
    llvm::APInt prefix_ap = eigenbits.extractBits(std_dim, n_qubits - std_dim);
    if (!prefixes_seen[prefix]) {
      if (!prefix_order.empty() &&
          prefix_ap.ult(prefix_order[prefix_order.size() - 1])) {
        prefixes_in_order = false;
      }
      prefix_order.push_back(
          eigenbits.extractBits(std_dim, n_qubits - std_dim));
    }
  ...
  for (qwerty::BasisVectorAttr vec : vl.getVectors()) {
    llvm::APInt eigenbits = vec.getEigenbits();
    llvm::APInt prefix = eigenbits.extractBits(std_dim, n_qubits - std_dim);
  ...
  if (prefixes_in_order) {
    ret = rewriter.getAttr<qwerty::BasisElemAttr>(
        rewriter.getAttr<qwerty::BuiltinBasisAttr>(vl.getPrimBasis(), std_dim));
  } else {
    llvm::SmallVector<qwerty::BasisVectorAttr> vecs;
...
qwerty::BasisVectorListAttr factorVeclist(
    mlir::RewriterBase &rewriter, std::deque<qwerty::BasisElemAttr> &dest_queue,
    qwerty::BasisVectorListAttr vl, qwerty::BasisVectorListAttr factor) {
  size_t n_qubits = vl.getDim();
  ...
  for (qwerty::BasisVectorAttr vec : vl.getVectors()) {
    llvm::APInt eigenbits = vec.getEigenbits();
    llvm::APInt prefix =
        eigenbits.extractBits(n_factor_qubits, n_qubits - n_factor_qubits);
...
qwerty::BasisVectorListAttr merge(mlir::RewriterBase &rewriter,
  ...
  for (qwerty::BasisVectorAttr lvec : left.getVectors()) {
    for (qwerty::BasisVectorAttr rvec : right.getVectors()) {
      llvm::APInt eigenbits = lvec.getEigenbits().concat(rvec.getEigenbits());
      vecs.push_back(rewriter.getAttr<qwerty::BasisVectorAttr>(
...
bool greedyMerge(mlir::RewriterBase &rewriter, qwerty::BasisVectorListAttr big,
  ...
  assert(small.getDim() < big.getDim() && "Merge called when not needed?");
  ...
  if (next.getDim() == delta) {
    ...
    small_rebuilt.push_back(rebuildZ(rewriter, merge(rewriter, small, next)));
    return true;
  } else if (next.getDim() < delta) {
    // continue, need to gobble more...
    qwerty::BasisVectorListAttr merged = merge(rewriter, small, next);
    return greedyMerge(rewriter, big, merged, big_queue, small_queue,
    ...
    if (stdnext) {
      qwerty::BasisVectorListAttr splat =
          splitStd(rewriter, small_queue, stdnext, delta).expandToVeclist();
      big_rebuilt.push_back(rebuildZ(rewriter, big));
      small_rebuilt.push_back(
          rebuildZ(rewriter, merge(rewriter, small, splat)));
      return true;
    } else { // vlnext
      qwerty::BasisVectorListAttr merged = merge(rewriter, small, vlnext);
...
struct Standardization {
  ...
  Standardization(qwerty::PrimitiveBasis prim_basis, size_t start, size_t end,
                  bool unconditional)
      : prim_basis(prim_basis), start(start), end(end),
  ...
  bool operator==(const Standardization &stdize) const {
    return prim_basis == stdize.prim_basis && stdize.start == start &&
           stdize.end == end;
  }

  bool isPadding() const { return prim_basis == (qwerty::PrimitiveBasis)-1; }

  size_t get_dim() const { return end - start; }
...
void findStandardizations(llvm::SmallVectorImpl<Standardization> &stdize,
...
// or unconditional.
void determineUnconditional(
    llvm::SmallVectorImpl<Standardization> &left_stdize,
  ...
  while (!lqueue.empty() && !rqueue.empty()) {
    ...
    if (left_dim == right_dim) {
      ...
      if (!right.isPadding()) {
        right_stdize.emplace_back(right.prim_basis, pos, pos + right_dim,
                                  unconditional);
      }
      pos += left_dim;
    } else {
      ...
      if (!big.isPadding() && primBasisIsSeparable(big.prim_basis)) {
        if (!small.isPadding()) {
          small_stdize.emplace_back(small.prim_basis, pos, pos + small_dim,
                                    unconditional);
        }
        big_stdize.emplace_back(big.prim_basis, pos, pos + small_dim,
                                unconditional);
        big_queue.push_front(
            Standardization(big.prim_basis, pos + small_dim, pos + big_dim));
      } else {
        if (!small.isPadding()) {
          small_stdize.emplace_back(small.prim_basis, pos, pos + small_dim,
                                    /*unconditional=*/false);
        ...
        big_queue.push_front(
            Standardization::new_padding(pos + small_dim, pos + big_dim));
      }
      pos += small_dim;
    }
  }
...
struct VectorPhase {
  ...
  size_t getEnd() const { return start + eigenbits.getBitWidth(); }
};
...
void findVectorPhases(llvm::SmallVectorImpl<VectorPhase> &vec_phases,
                      qwerty::BasisAttr basis, mlir::ValueRange basis_phases,
...
mlir::LogicalResult
rebuildAligned(mlir::Operation *offender, mlir::RewriterBase &rewriter,
               qwerty::BasisAttr basis_in, qwerty::BasisAttr basis_out,
  ...
  while (!left_queue.empty() && !right_queue.empty()) {
    ...
    if (left.getDim() == right.getDim()) {
      if (lstd && rstd) {
        ...
        if (rvl.isPredicate()) {
          return rewriter.notifyMatchFailure(
              offender, "right veclist is not full. Span checking bug?");
        }

        left_rebuilt.push_back(rebuildZ(rewriter, lstd.expandToVeclist()));
        right_rebuilt.push_back(rebuildZ(rewriter, rvl));
      } else if (rstd) { // && lvl
        if (lvl.isPredicate()) {
          return rewriter.notifyMatchFailure(
              offender, "left veclist is not full. Span checking bug?");
        }

        left_rebuilt.push_back(rebuildZ(rewriter, lvl));
        right_rebuilt.push_back(rebuildZ(rewriter, rstd.expandToVeclist()));
      } else { // lvl && rvl
        if ((lvl.isPredicate() || rvl.isPredicate()) &&
            (lvl.getPrimBasis() != rvl.getPrimBasis() ||
             !isPermutation(lvl.getVectors(), rvl.getVectors()))) {
          return rewriter.notifyMatchFailure(
              offender, "Veclists are not permutations of one another. "
      ...
      if (bigstd && smallstd) {
        big_rebuilt.push_back(rebuildZ(
            rewriter, splitStd(rewriter, big_queue, bigstd, small_dim)));
        small_rebuilt.push_back(rebuildZ(rewriter, smallstd));
      } else if (bigstd && smallvl) {
        big_rebuilt.push_back(
            rebuildZ(rewriter, splitStd(rewriter, big_queue, bigstd, small_dim)
                                   .expandToVeclist()));
        small_rebuilt.push_back(rebuildZ(rewriter, smallvl));
        if (smallvl.isPredicate()) {
          return rewriter.notifyMatchFailure(
              offender, "Small veclist is not full. Span checking broken?");
        }
      } else if (bigvl && smallstd) {
        if (qwerty::BasisElemAttr factored =
                factorFull(rewriter, big_queue, bigvl, small_dim)) {
          if (qwerty::BuiltinBasisAttr factored_std = factored.getStd()) {
            big_rebuilt.push_back(rebuildZ(rewriter, factored_std));
            small_rebuilt.push_back(rebuildZ(rewriter, smallstd));
          } else { // factored.getVeclist()
            qwerty::BasisVectorListAttr factored_vl = factored.getVeclist();
            big_rebuilt.push_back(rebuildZ(rewriter, factored_vl));
            small_rebuilt.push_back(
                rebuildZ(rewriter, smallstd.expandToVeclist()));
          }
        } else {
          if (!greedyMerge(rewriter, bigvl, smallstd.expandToVeclist(),
                           big_queue, small_queue, big_rebuilt,
                           small_rebuilt)) {
            return rewriter.notifyMatchFailure(offender, "Merging failed");
          }
        }
      } else { // bigvl && smallvl
        ...
        if (qwerty::BasisVectorListAttr factored =
          ...
          if (!greedyMerge(rewriter, bigvl, smallvl, big_queue, small_queue,
                           big_rebuilt, small_rebuilt)) {
            return rewriter.notifyMatchFailure(offender, "Merging failed");
          }
        }
  ...
  if (!left_queue.empty() || !right_queue.empty()) {
    return rewriter.notifyMatchFailure(
        offender, "Invalid translation. How did this pass span checking?");
  }

  return mlir::success();
}

bool shouldSkipStandardization(const Standardization &stdize,
                               bool unconditional) {
...
void compactStandardizedQubits(
    llvm::SmallVectorImpl<Standardization> &standardizations,
    bool unconditional, llvm::SmallVectorImpl<mlir::Value> &qubits,
...
void uncompactStandardizedQubits(
    llvm::SmallVectorImpl<Standardization> &standardizations,
    bool unconditional, llvm::SmallVectorImpl<mlir::Value> &qubits_out,
...
void standardizeCompressed(
  ...
  auto one_qubit_gate = [&](qcirc::Gate1Q kind, ssize_t ctrl_idx, size_t idx) {
    ...
    if (ctrl_idx >= 0) {
      ...
      gate = rewriter.create<qcirc::Gate1QOp>(loc, kind, controls, qubits[idx]);
    } else {
      gate = rewriter.create<qcirc::Gate1QOp>(loc, kind, control_qubits,
                                              qubits[idx]);
    }
  ...
  for (const Standardization &stdize : standardizations) {
    ...
    if (stdize.prim_basis == qwerty::PrimitiveBasis::FOURIER) {
      ...
      for (size_t i = qubit_idx; i < qubit_idx + dim; i++) {
        ...
        default:
          assert(0 &&
                 "Missing handling for PrimitiveBasis in standardization in "
...
void standardizeUncond(mlir::RewriterBase &rewriter, mlir::Location loc,
                       llvm::SmallVectorImpl<Standardization> &standardizations,
                       bool left, llvm::SmallVectorImpl<mlir::Value> &qubits) {
  llvm::SmallVector<mlir::Value> stdize_qubits;
  compactStandardizedQubits(standardizations, /*unconditional=*/true, qubits,
                            stdize_qubits);
  standardizeCompressed(rewriter, loc, standardizations, /*unconditional=*/true,
                        left, stdize_qubits);
  uncompactStandardizedQubits(standardizations, /*unconditional=*/true, qubits,
                              stdize_qubits);
}

// Conditional (de)standardizations. (See the "Standardization" subheading of
// Section 6.3 of the CGO paper.)
void standardizeCond(mlir::RewriterBase &rewriter, mlir::Location loc,
                     llvm::SmallVectorImpl<Standardization> &standardizations,
                     bool left, llvm::SmallVectorImpl<mlir::Value> &qubits,
                     llvm::SmallVectorImpl<qwerty::BasisElemAttr> &rebuilt) {
  llvm::SmallVector<mlir::Value> cond_qubits;
  compactStandardizedQubits(standardizations, /*unconditional=*/false, qubits,
                            cond_qubits);
  ...
  lowerPredBasisToInterleavedControls(
      ...
      [&](llvm::SmallVectorImpl<mlir::Value> &controls) {
        standardizeCompressed(rewriter, loc, standardizations,
                              /*unconditional=*/false, left, controls,
                              cond_qubits);
      });

  uncompactStandardizedQubits(standardizations, /*unconditional=*/false, qubits,
                              cond_qubits);
}
...
void chopOutRangeFromVeclist(
    mlir::Builder &builder, qwerty::BasisVectorListAttr vl,
    // Inclusive
    size_t chop_start,
    // Exclusive
    size_t chop_end,
  ...
  elems_out.push_back(builder.getAttr<qwerty::BasisElemAttr>(
      builder.getAttr<qwerty::BuiltinBasisAttr>(qwerty::PrimitiveBasis::Z,
                                                pad_dim)));
...
void chopOutRange(mlir::Builder &builder, qwerty::BasisElemAttr elem,
                  // Inclusive
                  size_t chop_start,
                  // Exclusive
                  size_t chop_end,
...
qwerty::BasisAttr
replaceVecWithPadding(mlir::Builder &builder, const VectorPhase &vp,
                      llvm::SmallVectorImpl<qwerty::BasisElemAttr> &rebuilt) {
  ...
  while (rebuilt_idx < rebuilt.size() && qubit_idx < vp.getEnd()) {
    qwerty::BasisElemAttr elem = rebuilt[rebuilt_idx];
    size_t dim = elem.getDim();

    size_t chop_start = vp.start < qubit_idx ? 0 : vp.start - qubit_idx;
    size_t chop_end =
        qubit_idx + dim < vp.getEnd() ? dim : vp.getEnd() - qubit_idx;
...
void impartVecPhases(mlir::RewriterBase &rewriter, mlir::Location loc,
                     llvm::SmallVectorImpl<VectorPhase> &phases, bool negate,
                     llvm::SmallVectorImpl<mlir::Value> &qubits,
                     llvm::SmallVectorImpl<qwerty::BasisElemAttr> &rebuilt) {
  for (const VectorPhase &vp : phases) {
    ...
    llvm::SmallVector<mlir::Value> vec_qubits(qubits.begin() + vp.start,
                                              qubits.begin() + vp.getEnd());
    for (size_t bit = 0; bit < vp.eigenbits.getBitWidth(); bit++) {
      size_t qubit_idx = vec_qubits.size() - 1 - bit;
      if (!vp.eigenbits[bit]) {
        vec_qubits[qubit_idx] =
            rewriter
                .create<qcirc::Gate1QOp>(loc, qcirc::Gate1Q::X,
                                         mlir::ValueRange(),
                                         vec_qubits[qubit_idx])
    ...
    if (negate) {
      theta = wrapStationaryFloatOps(
          rewriter, loc, theta, [&](mlir::ValueRange args) {
            assert(args.size() == 1);
            mlir::Value theta_arg = args[0];
            return rewriter.create<mlir::arith::NegFOp>(loc, theta_arg)
                .getResult();
    ...
    lowerPredBasisToInterleavedControls(
        ...
        [&](llvm::SmallVectorImpl<mlir::Value> &controls) {
          ...
          qcirc::Gate1Q1POp gate = rewriter.create<qcirc::Gate1Q1POp>(
              loc, qcirc::Gate1Q1P::P, theta, p_controls,
...
// predicate.
void shootdownStandardizations(
    llvm::SmallVectorImpl<Standardization> &left_stdize,
    llvm::SmallVectorImpl<Standardization> &right_stdize,
  ...
  for (;;) {
    ...
    bool has_phase = (left_phase_idx < left_phases.size() &&
    ...
    while (!done) {
      ...
      size_t rebuilt_end = qubit_idx + left_rebuilt[rebuilt_idx].getDim();
...
// TODO:
// Create both helper functions for forward synthesis
// which is std**N >> std**(N-1) // std.revolve
...
struct ArbitraryBasisRevolveGenerator
  ...
  matchAndRewrite(qwerty::QBundleBasisTranslationOp trans, OpAdaptor adaptor,
    ...
    // but no requirement for BuiltinBasisAttr because baz
    // can be any basis as long as dim matches
    ...
    qwerty::BasisVectorListAttr veclist_outer;
    qwerty::ApplyRevolveGeneratorAttr revolve_outer;
    qwerty::BuiltinBasisAttr builtin_outer;
    qwerty::ApplyRevolveGeneratorAttr revolve;

    bool inverse = false;

    // To get dim for baz in baz >> foo // bar.revolve
    uint64_t baz_dim = 0;
    if (elt_out.getRevolve()) {
    ...
    qwerty::BasisAttr baz_N =
        qwerty::BasisAttr::get(rewriter.getContext(), {baz_elem});

    // foo // bar.revolve basis
    qwerty::BasisElemAttr revolve_elem =
        qwerty::BasisElemAttr::get(rewriter.getContext(), revolve);
    ...
    if (!inverse) {
      qwerty::QBundleBasisTranslationOp baz_to_std =
          rewriter.create<qwerty::QBundleBasisTranslationOp>(
              loc, baz_N, std_N, mlir::ValueRange(),
              trans.getQbundleIn());
      ...
      qwerty::QBundleBasisTranslationOp std_to_baz =
          rewriter.create<qwerty::QBundleBasisTranslationOp>(
              loc, std_N, baz_N, mlir::ValueRange(),
...
struct ArbitraryRevolveBasisRevolveGenerator
  ...
  matchAndRewrite(qwerty::QBundleBasisTranslationOp trans, OpAdaptor adaptor,
    ...
    // check whether revolve's bv1 and bv2 are 0, 1
    ...
    // for use in std >> bar, or reverse
    auto bar_1 = rewriter.getAttr<qwerty::BasisAttr>(
        std::initializer_list<qwerty::BasisElemAttr>{bv1_bv2_elem});
    ...
    // Now we can create two basis translations to create
    // std**N >> foo // std.revolve | id**(N-1) * (std >> bar)
    if (!inverse) {
      // Steps:
      ...
      qwerty::QBundleBasisTranslationOp std_to_bar =
          rewriter.create<qwerty::QBundleBasisTranslationOp>(
              loc, std_1, bar_1, mlir::ValueRange(),
              packed_last_qubit);
...
// This pattern matches on either:
// std**N >> foo // std.revolve OR foo // std.revolve >> std**N
// and creates either:
// std**N >> std**(N-1) // std.revolve | RotL | (std**(N-1) >> foo) * id OR
// (foo >> std**(N-1)) * id | RotR | std**(N-1) // std.revolve >> std**N
// respectively.
//
// Note that the RotL and RotR are implicit, so the base case
...
struct AlignBasisTranslations
  ...
  matchAndRewrite(qwerty::QBundleBasisTranslationOp trans, OpAdaptor adaptor,
    ...
    findVectorPhases(right_phases, basis_out, basis_phases,
    ...
    shootdownStandardizations(left_stdize, right_stdize, left_phases,
                              right_phases, left_rebuilt, right_rebuilt);
    ...
    mlir::Value after_rebuilt =
        rewriter
            .create<qwerty::QBundleBasisTranslationOp>(
                loc, rebuilt_in, rebuilt_out, mlir::ValueRange(), left_repacked)
            .getQbundleOut();
...
void synthesizePermutationSlow(
    mlir::RewriterBase &rewriter, mlir::Location loc,
    llvm::SmallVectorImpl<mlir::Value> &control_qubits,
    llvm::SmallVectorImpl<mlir::Value> &qubits, size_t qubit_idx,
    qwerty::BasisVectorListAttr left, qwerty::BasisVectorListAttr right) {
  ...
  for (size_t i = 0; i < left.getVectors().size(); i++) {
    ...
    // endianness as we do
    uint32_t l_idx =
        static_cast<uint32_t>(lv.getEigenbits().reverseBits().getZExtValue());
    uint32_t r_idx =
        static_cast<uint32_t>(rv.getEigenbits().reverseBits().getZExtValue());

    perm[l_idx] = r_idx;
  }

  qcirc::synthPermutationSlow(rewriter, loc, control_qubits, qubits, qubit_idx,
                              perm);
}
...
void synthesizePermutationFast(
  ...
  for (auto [left_vec, right_vec] :
       llvm::zip(left.getVectors(), right.getVectors())) {
  ...
  qcirc::synthPermutationFast(rewriter, loc, control_qubits, qubits, qubit_idx,
                              perm);
}

// Synthesize the reversible permutation of basis states at the heart of a
// basis translation circuit.
void synthesizePermutation(mlir::RewriterBase &rewriter, mlir::Location loc,
                           llvm::SmallVectorImpl<mlir::Value> &control_qubits,
                           llvm::SmallVectorImpl<mlir::Value> &qubits,
                           size_t qubit_idx, qwerty::BasisVectorListAttr left,
                           qwerty::BasisVectorListAttr right) {
  ...
  if (dim <= 10) {
    synthesizePermutationSlow(rewriter, loc, control_qubits, qubits, qubit_idx,
                              left, right);
  } else {
    synthesizePermutationFast(rewriter, loc, control_qubits, qubits, qubit_idx,
                              left, right);
  }
}
      ...
      if (left.getStd()) { // && right.getStd()
        ...
        if (lvl == rvl) {
          ...
          llvm::SmallVector<qwerty::BasisElemAttr> other_elems(
              basis_in.getElems().begin(), basis_in.getElems().begin() + i);
          ...
          other_elems.push_back(rewriter.getAttr<qwerty::BasisElemAttr>(
              rewriter.getAttr<qwerty::BuiltinBasisAttr>(
                  qwerty::PrimitiveBasis::Z, dim)));
          ...
          lowerPredBasisToInterleavedControls(
              ...
              [&](llvm::SmallVectorImpl<mlir::Value> &controls) {
                synthesizePermutation(rewriter, loc, controls, qubits,
                                      qubit_idx, lvl, rvl);
              });
        }
      }
...
struct QBundleMeasureNonStd
    : public mlir::OpConversionPattern<qwerty::QBundleMeasureOp> {
  using mlir::OpConversionPattern<
      qwerty::QBundleMeasureOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleMeasureOp meas, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    ...
    if (elems.size() == 1 && elems[0].getStd() &&
        elems[0].getStd().getPrimBasis() == qwerty::PrimitiveBasis::Z) {
      return mlir::failure();
    }

    uint64_t dim = basisAttr.getDim();
    qwerty::BuiltinBasisAttr sba1 = rewriter.getAttr<qwerty::BuiltinBasisAttr>(
        qwerty::PrimitiveBasis::Z, dim);
...
struct QBundleMeasureOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleMeasureOp> {
  using mlir::OpConversionPattern<
      qwerty::QBundleMeasureOp>::OpConversionPattern;
  ...
  matchAndRewrite(qwerty::QBundleMeasureOp meas, OpAdaptor adaptor,
    ...
    if (basis.getElems().size() != 1 || !basis.getElems()[0].getStd() ||
        basis.getElems()[0].getStd().getPrimBasis() !=
    ...
    mlir::ValueRange unpacked_range =
        rewriter.create<qwerty::QBundleUnpackOp>(loc, meas.getQbundle())
            .getQubits();
...
struct QBundleProjectNonStd
    : public mlir::OpConversionPattern<qwerty::QBundleProjectOp> {
  using mlir::OpConversionPattern<
      qwerty::QBundleProjectOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleProjectOp proj, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    ...
    qwerty::QBundleProjectOp proj2 = rewriter.create<qwerty::QBundleProjectOp>(
...
struct QBundleProjectOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleProjectOp> {
  using mlir::OpConversionPattern<
      qwerty::QBundleProjectOp>::OpConversionPattern;
  ...
  matchAndRewrite(qwerty::QBundleProjectOp proj, OpAdaptor adaptor,
    ...
    mlir::ValueRange unpacked_range =
        rewriter.create<qwerty::QBundleUnpackOp>(loc, proj.getQbundleIn())
            .getQubits();
    llvm::SmallVector<mlir::Value> unpacked(unpacked_range);
...
struct QBundleFlipOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleFlipOp> {
  using mlir::OpConversionPattern<qwerty::QBundleFlipOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleFlipOp flip, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    ...
    if (basis.getDim() != 1) {
      return rewriter.notifyMatchFailure(
          flip, "Currently only 1D bases are supported in .flip");
    }
    if (basis.hasPredicate()) {
      return rewriter.notifyMatchFailure(
          flip, "Predicates are not supported for .flip");
    }

    assert(basis.getElems().size() == 1 &&
           "1D basis has more than 1 element, how?");
    ...
    qwerty::BasisAttr rev_basis = rewriter.getAttr<qwerty::BasisAttr>(
        rewriter.getAttr<qwerty::BasisElemAttr>(
            rewriter.getAttr<qwerty::BasisVectorListAttr>(
                std::initializer_list<qwerty::BasisVectorAttr>{vectors[1],
                                                               vectors[0]})));
...
struct QBundleRotateOpLowering
    : public mlir::OpConversionPattern<qwerty::QBundleRotateOp> {
  using mlir::OpConversionPattern<qwerty::QBundleRotateOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(qwerty::QBundleRotateOp rot, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    ...
    if (basis.getDim() != 1) {
      return rewriter.notifyMatchFailure(
          rot, "Currently only 1D bases are supported in .rotate");
    }
    if (basis.hasPredicate()) {
      return rewriter.notifyMatchFailure(
          rot, "Predicates are not supported for .rotate");
    }
    ...
    mlir::Value theta_by_2 = wrapStationaryFloatOps(
        rewriter, loc, rot.getTheta(), [&](mlir::ValueRange args) {
          assert(args.size() == 1);
          mlir::Value theta_arg = args[0];
          mlir::Value const_2 = rewriter
                                    .create<mlir::arith::ConstantOp>(
                                        loc, rewriter.getF64FloatAttr(2.0))
                                    .getResult();
          return rewriter.create<mlir::arith::DivFOp>(loc, theta_arg, const_2)
              .getResult();
        });
    mlir::Value neg_theta_by_2 = wrapStationaryFloatOps(
        rewriter, loc, theta_by_2, [&](mlir::ValueRange args) {
          assert(args.size() == 1);
          mlir::Value theta_by_2_arg = args[0];
          return rewriter.create<mlir::arith::NegFOp>(loc, theta_by_2_arg)
              .getResult();
        });
        ...
        rewriter.getAttr<qwerty::BasisElemAttr>(
            rewriter.getAttr<qwerty::BasisVectorListAttr>(
                std::initializer_list<qwerty::BasisVectorAttr>{
                    rewriter.getAttr<qwerty::BasisVectorAttr>(
                        vectors[0].getPrimBasis(), vectors[0].getEigenbits(),
                        vectors[0].getDim(), /*hasPhase=*/true),
                    rewriter.getAttr<qwerty::BasisVectorAttr>(
                        vectors[1].getPrimBasis(), vectors[1].getEigenbits(),
                        vectors[1].getDim(), /*hasPhase=*/true)})));
...
struct QwertyToQCircConversionPass
    : public qwerty::QwertyToQCircConversionBase<QwertyToQCircConversionPass> {
  void runOnOperation() override {
    ...
    if (mlir::failed(generateFuncSpecs(module_op, usages, type_converter))) {
    ...
    target.addLegalDialect<mlir::BuiltinDialect, qcirc::QCircDialect,
    ...
    // the right types
    target.addDynamicallyLegalOp<mlir::scf::IfOp, mlir::scf::YieldOp,
                                 mlir::arith::SelectOp, qcirc::CalcOp,
                                 qcirc::CalcYieldOp>(
    ...
    patterns.add<
        ...
        CalcYieldOpTypeFix, QBundlePackOpLowering, QBundleUnpackOpLowering,
        BitBundlePackOpLowering, BitBundleUnpackOpLowering, FuncOpLowering,
        ReturnOpLowering, CallOpLowering, CallIndirectOpLowering,
        FuncConstOpLowering, FuncAdjointOpLowering, FuncPredOpLowering,
        ...
        QBundlePhaseOpLowering, QBundleIdentityOpLowering, ArbitraryBasisRevolveGenerator,
        ArbitraryRevolveBasisRevolveGenerator, RecurseRevolveBasisGenerator,
        LowerRevolveBasisGenerator, AlignBasisTranslations, SynthesizePermutations,
        QBundleMeasureNonStd, QBundleMeasureOpLowering, QBundleProjectNonStd,
        QBundleProjectOpLowering, QBundleFlipOpLowering,
        QBundleRotateOpLowering, SuperposOpLowering, EnsembleOpLowering>(
        type_converter, &getContext());

    if (mlir::failed(mlir::applyFullConversion(module_op, target,
                                               std::move(patterns)))) {
      signalPassFailure();
    }
  }
