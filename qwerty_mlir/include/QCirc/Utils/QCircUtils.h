...
// Synthesize a classical permutation as QCirc IR, inserting QCirc ops are
// inline wherever the builder's insertion point is, without creating a new
// FuncOp. The `init_control_qubits` argument adds additional controls to each
// gate. Per the tweedledum gods, `perm` means the following:
// > A permutation is specified as a vector of `2^n` different integers
// > ranging from `0` to `2^n-1`.
void synthPermutation(
        mlir::OpBuilder &builder,
        mlir::Location loc,
        llvm::SmallVectorImpl<mlir::Value> &init_control_qubits,
